...
import { generateQueryBreedingReportService } from 'services/report/inspection-management';
import { WEB_ROUTES, FUNCTION_NAMES } from 'constants/index';

const QueryBreedingReport = (props) => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.QUERY_BREEDING.name}`;
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.QUERY_BREEDING.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = () => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateQueryBreedingReportService({ reportParameters, password }), onPending, onSuccess, onError);
  };

  const checkMandatory = startDate && endDate && vectorTypeList.length > 0;

  return (
    <>
      <Header />
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.QUERY_BREEDING.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.QUERY_BREEDING]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.QUERY_BREEDING.name}</h1>
          </div>
          ...
          <div className="block-summary filterMainWrapper tabsContainer" style={{ padding: '30px' }}>
            <div className="tab-pane__group bg-white">
              <div className="card">
                ...
                <b className="text-body">Sort Options (only select one option type).</b>
                <div className="form-inline mb-4">
                  <div className="form-group form-group__stacked mb-4">
                  ...
                  <div className="form-group form-group__stacked flex-column">
                    <b className="text-body">Sample Id</b>
                    <input
                      className="textfield"
                      onChange={(event) => {
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateQueryBreeding} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(QueryBreedingReport));
