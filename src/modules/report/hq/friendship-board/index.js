...
import { generateFriendshipBoardReportService } from 'services/report/hq';
import { weekNumberLOVs, yearNumberLOVs, WEB_ROUTES, FUNCTION_NAMES } from 'constants/index';

const HQFriendshipBoard = () => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.FRIENDSHIP_BOARD.name}`;
  }, []);
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.FRIENDSHIP_BOARD.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = (error) => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateFriendshipBoardReportService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.FRIENDSHIP_BOARD.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.FRIENDSHIP_BOARD]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.FRIENDSHIP_BOARD.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateFriendshipBoard} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(HQFriendshipBoard));
