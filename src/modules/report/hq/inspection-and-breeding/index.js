...
import { generateInspectionAndBreedingReportService } from 'services/report/hq';
import { WEB_ROUTES, FUNCTION_NAMES } from 'constants/index';

const HQInspectionAndBreeding = (props) => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.INSPECTION_AND_BREEDING.name}`;
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.INSPECTION_AND_BREEDING.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = (error) => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateInspectionAndBreedingReportService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.INSPECTION_AND_BREEDING.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.INSPECTION_AND_BREEDING]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.INSPECTION_AND_BREEDING.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateInspectionAndBreeding} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(HQInspectionAndBreeding));
