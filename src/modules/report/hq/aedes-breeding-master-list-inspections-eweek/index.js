...
import { generateBreedingInspectionEweekReportService } from 'services/report/hq';
import { weekNumberLOVs, yearNumberLOVs, WEB_ROUTES, FUNCTION_NAMES } from 'constants/index';

const HQBreedingMasterListInspectionEweek = (props) => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.AEDES_BREEDING_MASTER_LIST_INSPECTIONS_EWEEK.name}`;
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.AEDES_BREEDING_MASTER_LIST_INSPECTIONS_EWEEK.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = (error) => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateBreedingInspectionEweekReportService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.AEDES_BREEDING_MASTER_LIST_INSPECTIONS_EWEEK.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.AEDES_BREEDING_MASTER_LIST_INSPECTIONS_EWEEK]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.AEDES_BREEDING_MASTER_LIST_INSPECTIONS_EWEEK.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateABMLeweekInspections} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(HQBreedingMasterListInspectionEweek));
