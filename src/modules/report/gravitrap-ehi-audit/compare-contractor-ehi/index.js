...
import { generateCompareContractorEHIService } from 'services/report/gravitrap-ehi-audit';
...
const CompareContractorEHI = () => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.COMPARE_CONTRACTOR_EHI.name}`;
  }, []);
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.COMPARE_CONTRACTOR_EHI.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = () => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateCompareContractorEHIService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.COMPARE_CONTRACTOR_EHI.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.COMPARE_CONTRACTOR_EHI]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.COMPARE_CONTRACTOR_EHI.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateCompareContractorEHI} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
