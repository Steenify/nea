...
import { generateMosquitoIdService } from 'services/report/gravitrap-ehi-audit';
...
const CompareContractorEHI = () => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.MOSQUITO_ID_REPORT.name}`;
  }, []);
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.MOSQUITO_ID_REPORT.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = () => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateMosquitoIdService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.MOSQUITO_ID_REPORT.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.MOSQUITO_ID_REPORT]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.MOSQUITO_ID_REPORT.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateMosquitoID} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
