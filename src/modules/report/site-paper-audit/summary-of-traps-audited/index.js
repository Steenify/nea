...
import { generateSummaryOfTrapsAuditedService } from 'services/report/site-paper-audit';

const SummaryOfTrapsAudited = (props) => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.SUMMARY_OF_TRAPS_AUDITED.name}`;
  ...
  const onSearch = (password) => {
    const reportParameters = [
      { name: 'AuditFrom', value: startDate ? dateStringFromDate(startDate) : '' },
      { name: 'AuditTo', value: endDate ? dateStringFromDate(endDate) : '' },
      { name: 'RO', value: roList[0]?.value || '' },
      { name: 'GRC', value: grcList[0]?.value || '' },
      { name: 'Division', value: divisionList[0]?.value || '' },
    ];
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.SUMMARY_OF_TRAPS_AUDITED.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = (error) => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateSummaryOfTrapsAuditedService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.SUMMARY_OF_TRAPS_AUDITED.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.SUMMARY_OF_TRAPS_AUDITED]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.SUMMARY_OF_TRAPS_AUDITED.name}</h1>
          </div>
            ...
            <div className="tab-pane__group bg-white">
              <div className="card">
                <div className="form-inline mb-4">
                  <div>
                    <b className="text-body">Audit From</b>
                    <SingleDatePickerV2
                  ...
                  <div>
                    <b className="text-body">Audit To</b>
                    <SingleDatePickerV2
                      className="mt-2 mr-2 xs-paddingBottom15"
                      date={endDate}
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateSummaryOfTrapsAudited} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(SummaryOfTrapsAudited));
