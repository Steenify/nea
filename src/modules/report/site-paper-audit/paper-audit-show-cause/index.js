...
import { generatePaperAuditShowCauseService } from 'services/report/site-paper-audit';
import { WEB_ROUTES, weekNumberLOVs, yearNumberLOVs, FUNCTION_NAMES } from 'constants/index';

const PaperAuditShowCause = () => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.PAPER_AUDIT_SHOW_CAUSE_REPORT.name}`;
  }, []);
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.PAPER_AUDIT_SHOW_CAUSE_REPORT.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = (error) => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generatePaperAuditShowCauseService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.PAPER_AUDIT_SHOW_CAUSE_REPORT.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.PAPER_AUDIT_SHOW_CAUSE_REPORT]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.PAPER_AUDIT_SHOW_CAUSE_REPORT.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generatePaperAuditShowCause} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(PaperAuditShowCause));
