...
import { generateMissedMaintenanceTrapService } from 'services/report/site-paper-audit';
import { WEB_ROUTES, weekNumberLOVs, yearNumberLOVs, FUNCTION_NAMES } from 'constants/index';

const MissedMaintenanceTrap = () => {
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.REPORT.DISCREPANCY_NUMBER_OF_TRAPS.name}`;
  }, []);
  ...
  const onSearch = (password) => {
    ...
    const onSuccess = (data) => {
      ...
      autoGenerateDownloadLink(`${WEB_ROUTES.REPORT.DISCREPANCY_NUMBER_OF_TRAPS.name}.${extension}`, mimeType, byteArrayToBase64(reportData));
      setLocalLoading(false);
    };
    const onError = (error) => {
      setLocalLoading(false);
    };
    return actionTryCatchCreator(generateMissedMaintenanceTrapService({ reportParameters, password }), onPending, onSuccess, onError);
  };
  ...
  return (
    <>
      ...
      <div className="main-content">
        <NavBar active={WEB_ROUTES.REPORT.DISCREPANCY_NUMBER_OF_TRAPS.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.REPORT, WEB_ROUTES.REPORT.DISCREPANCY_NUMBER_OF_TRAPS]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.REPORT.DISCREPANCY_NUMBER_OF_TRAPS.name}</h1>
          </div>
          ...
          <Footer />
          <CheckForEncryption functionName={FUNCTION_NAMES.generateMissedMaintenanceTrap} ref={passwordModalRef} onGenerate={onSearch} />
        </div>
      </div>
    </>
  );
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(MissedMaintenanceTrap));
