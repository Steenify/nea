...
const ShowCauseReport = (props) => {
  ...
  const getCompanyNamesAction = async (callback) => {
    const onPending = () => {
      setIsLoading(true);
    };
    const onSuccess = (res) => {
      ...
      if (callback) callback(companyNames);
    };
    const onError = (e) => {
      setIsLoading(false);
    };

    await actionTryCatchCreator(companyNamesListingService(), onPending, onSuccess, onError);
  };
