...
const TownCouncilFineRegime = (props) => {
  const {
    getListAction,
    filterListAction,
  ...
  useEffect(() => {
    document.title = `NEA | ${WEB_ROUTES.INSPECTION_MANAGEMENT.TOWN_COUNCIL_FINE_REGIME.name}`;
  ...
  const getTrProps = (_state, rowInfo) => {
    if (rowInfo && rowInfo.row) {
      let detailAction = '';
      if (functionNameList.includes(FUNCTION_NAMES.confirmTcRegime)) {
        const { immediateFine, thresholdList, noEnforcement } = rowInfo.row;
        if ((!immediateFine && !thresholdList && !noEnforcement) || immediateFine === '-' || thresholdList === '-' || noEnforcement === '-') detailAction = 'confirm';
      }
      if (functionNameList.includes(FUNCTION_NAMES.supportTcRegime)) {
        detailAction = 'support';
      }
      if (functionNameList.includes(FUNCTION_NAMES.approveTcRegime)) {
      ...
      if (functionNameList.includes(FUNCTION_NAMES.getTcRegimeDetail) || functionNameList.includes(FUNCTION_NAMES.getTcRegimeSummary)) {
        return {
          onClick: () => {
            history.push(`${WEB_ROUTES.INSPECTION_MANAGEMENT.TOWN_COUNCIL_FINE_REGIME_DETAIL.url}`, {
  ...
  return (
    <>
      ...
      <div className="main-content workspace__main">
        <NavBar active={WEB_ROUTES.INSPECTION_MANAGEMENT.TOWN_COUNCIL_FINE_REGIME.name} />
        <div className="contentWrapper">
          <NewBreadCrumb page={[WEB_ROUTES.INSPECTION_MANAGEMENT, WEB_ROUTES.INSPECTION_MANAGEMENT.TOWN_COUNCIL_FINE_REGIME]} />
          <div className="main-title">
            <h1>{WEB_ROUTES.INSPECTION_MANAGEMENT.TOWN_COUNCIL_FINE_REGIME.name}</h1>
          </div>
...
const mapStateToProps = ({ global, vectorInspectionReducers: { townCouncilFineRegime } }, ownProps) => ({
  ...ownProps,
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(TownCouncilFineRegime));
